...
public abstract class Polygon extends Shape {
	...
	/*
	 * Everything below this point is completely independent of any particular
	 * polygon. You can tell by the fact that the code does not mention any members
	 * (fields or methods) from the object itself.
	 */
	...
	private Point calculateCornerPoint(Point center, int[] offset, double alpha) {
		return rotatePoint(center, alpha, movePoint(center, offset));
	}
	...
	private Point rotatePoint(Point center, double alpha, Point newCorner) {
		double newX = center.x + (newCorner.x - center.x) * Math.cos(alpha)
				- (newCorner.y - center.y) * Math.sin(alpha);
		double newY = center.y + (newCorner.x - center.x) * Math.sin(alpha)
				+ (newCorner.y - center.y) * Math.cos(alpha);
